...
public class IndicatorBuilderTest extends BqTest {
  ...
  "fn" : "close_price"} ]
		...
		for (int i=series.getBeginIndex(); i<=series.getEndIndex();i++) {
			Assertions.assertThat(indicator.getValue(i).doubleValue()).isEqualByComparingTo(series.getBar(i).getClosePrice().doubleValue());
		}
	...
	void checkSameOutput(String expression, Indicator<Num> refIndicator) {
		...
		for (int i=barSeries.getBeginIndex(); i<=barSeries.getEndIndex(); i++) {
			...
			Assertions.assertThat(testNum.doubleValue()).withFailMessage("expression=%s indicator=%s index=%d", expression,testIndicator,i).isEqualByComparingTo(refNum.doubleValue());
		}
  ...
  "fn" : "sma", "args":[5]} ]
		...
		for (int i=series.getBeginIndex(); i<=series.getEndIndex();i++) {
			Assertions.assertThat(indicator.getValue(i).doubleValue()).isEqualByComparingTo(refIndicator.getValue(i).doubleValue());
		}
	...
	public void testSame() {
		...
		checkSameOutput("difference(close_price)", new DifferenceIndicator(new ClosePriceIndicator(series)));
		checkSameOutput("difference", new DifferenceIndicator(new ClosePriceIndicator(series)));
		checkSameOutput("difference(high_price)", new DifferenceIndicator(new HighPriceIndicator(series)));
