...
public class IndicatorBuilder {
	...
	private static Supplier<Map<String,Class<Indicator<?>>>> mapSupplier = Suppliers.memoize(IndicatorBuilder::buildMap);
	protected Map<String,Class<Indicator<?>>> getIndicatorNameMap() {
		return mapSupplier.get();
	}
	static private Map<String,Class<Indicator<?>>> buildMap() {

		Map<String,Class<Indicator<?>>> map = Maps.newHashMap();

		try (ScanResult scanResult = new ClassGraph().enableClassInfo().enableMethodInfo().acceptPackages("org.ta4j")
	 ...
	 List<String> tokenize(String s) {
		...
		return Lists.newArrayList(result.stream().map(it -> it.trim()).filter(it -> S.isNotBlank(it)).toList());

	}

	enum TokenType {
		IDENTIFIER, INTEGER, DOUBLE, OPEN_PAREN, CLOSE_PAREN, COMMA, WHITESPACE
	}
	...
	private List<Object> buildArgs(JsonNode functionNode, BarSeries series) {
		...
		functionNode.path("args").forEach(arg->{
				...
				if (arg.isLong()) {
					...
					throw new IllegalArgumentException("unsupported arg: "+arg+" "+arg.getNodeType());
				}
	...
	protected Indicator<Num> create(JsonNode input, BarSeries series) {
		...
		if (v.isNumber()) {
			ConstantIndicator<Num> indicator = new ConstantIndicator<Num>(series, DoubleNum.valueOf(v.asDouble()));
			return indicator;
		}
	...
	private Indicator<Num> invokeBestConstructor(Class<Indicator<?>> clazz, List<Object> args, BarSeries bs) {
	
		
		var ctors = List.of(clazz.getDeclaredConstructors());	
		ctors = List.of(clazz.getDeclaredConstructors()).stream().filter(ctor->{
		...
		if (args.size()>0 && (args.get(0) instanceof Indicator)) {
			// the user selected an indicator as the first arg.  Implicitly adding the close price doesn't make sense
		...
		ctors = List.of(clazz.getDeclaredConstructors()).stream().filter(ctor->{
			int arity = ctor.getParameterCount();
			if (arity==implicitArgs.size() && Indicator.class.isAssignableFrom( ctor.getParameterTypes()[0])) {
				return true;
			}
			return false;
		...
		ctors = List.of(clazz.getDeclaredConstructors()).stream().filter(ctor->{
			int arity = ctor.getParameterCount();
			if (arity==implicitArgs.size() && BarSeries.class.isAssignableFrom( ctor.getParameterTypes()[0])) {
				return true;
			}
			return false;
