...
public class BasicOHLCV implements OHLCV {
	...
	public static OHLCV of(LocalDate t, Double open, Double high, Double low, Double close, Double vol) {
		return of(t.atStartOfDay(Zones.UTC).toInstant(), 
	...
	public static OHLCV of(LocalDate t, BigDecimal open, BigDecimal high, BigDecimal low, BigDecimal close,
			BigDecimal vol) {
		return of(t.atStartOfDay(Zones.UTC).toInstant(), open, high, low, close, vol);
	}

	public static OHLCV of(Instant t, BigDecimal open, BigDecimal high, BigDecimal low, BigDecimal close,
			BigDecimal volume) {
	...
	public String toString() {
		...
		if (ts.atZone(Zones.UTC).toLocalDate().atStartOfDay(Zones.UTC).toEpochSecond() == ts.getEpochSecond()) {
			datePart = ts.atZone(Zones.UTC).toLocalDate().toString();
		} else {
			datePart = ts.toString();
		}
		return MoreObjects.toStringHelper("OLHCV").add("date", datePart).add("o", getOpen().orElse(null))
				.add("h", getHigh().orElse(null)).add("l", getLow().orElse(null)).add("c", getClose().orElse(null))
				.add("v", getVolume().orElse(null)).toString();
	}
}
