...
import static bx.util.S.notBlank;
...
public class MassiveProvider extends DataProvider {
	...
	protected Stream<OHLCV> fetch(Request request) {
		...
		if (request.from==null) {
			from = LocalDate.now().minusDays(365*10).toString();		
		}
		else {
		...
		String url = String.format("https://api.massive.com/v2/aggs/ticker/%s/range/1/day/%s/%s?adjusted=true&sort=asc",
		...
		kong.unirest.core.HttpResponse<tools.jackson.databind.JsonNode> response = Unirest.get(url)
	...
	OHLCV toOHLCV(JsonNode n) {
		...
		var d = ZonedDateTime.ofInstant(Instant.ofEpochMilli(n.path("t").asLong()), Zones.NYC).toLocalDate();

		;
		return BasicOHLCV.of(d, (BigDecimal) n.path("o").asDecimalOpt().orElse(null),
